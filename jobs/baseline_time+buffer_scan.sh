...
#SBATCH --mem=16G
#SBATCH --output=logs/baseline_time+buffer_scan_threaded_%A_%a.out
#SBATCH --error=logs/baseline_time+buffer_scan_threaded_%A_%a.err
...
OUTPUT_DIR=${OUTPUT_DIR:-results/baseline/ft/time+buffer_scan}
...
CLEANUP_TIMES=(${CLEANUP_TIME_LIST:-1 2 3})
BUFFER_DEPTHS=(${BUFFER_DEPTH_LIST:-1 2 3})
