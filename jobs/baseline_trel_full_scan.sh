#!/bin/bash
#SBATCH --job-name=baseline_trel_full_scan_threaded
...
#SBATCH --mem=32G
#SBATCH --output=logs/baseline_trel_full_scan_threaded_%A_%a.out
#SBATCH --error=logs/baseline_trel_full_scan_threaded_%A_%a.err
...
OUTPUT_DIR=${OUTPUT_DIR:-results/baseline/trel/full_scan}
...
if [[ -z "${SLURM_ARRAY_TASK_ID:-}" ]]; then
    ...
    echo "Submitting ${TOTAL_TASKS} baseline trel tasks: mode=${MODE}, repeats=${NREPEATS}, threads/task=${THREADS_PER_TASK}"
