#!/bin/bash
#SBATCH --job-name=cnot_sheetcopy_initial_scan_threaded
...
#SBATCH --mem=32G
#SBATCH --output=logs/cnot_sheetcopy_initial_scan_threaded_%A_%a.out
#SBATCH --error=logs/cnot_sheetcopy_initial_scan_threaded_%A_%a.err
...
# A light first pass over the sheet-copy CNOT simulator. Override any of these
...
if [[ -z "${SLURM_ARRAY_TASK_ID:-}" ]]; then
    ...
    echo "Submitting ${TOTAL_TASKS} sheet-copy CNOT initial-scan tasks"
...
export OUT_ADJ="_cnot_sheetcopy_initial_p${PVAL}_L${LVAL}_rep${REPEAT_INDEX}_T${TVAL_VAL}_Tpre${T_PRE_VAL}_Tpost${T_POST_VAL}${SAMPLE_ADJ}_thr${JULIA_NUM_THREADS}"
