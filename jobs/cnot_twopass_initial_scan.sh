#!/bin/bash
#SBATCH --job-name=cnot_twopass_full_scan
...
#SBATCH --mem=32G
#SBATCH --output=logs/cnot_twopass_initial_scan_%A_%a.out
#SBATCH --error=logs/cnot_twopass_initial_scan_%A_%a.err
...
NREPEATS=${2:-1}
MAX_CONCURRENT=${3:-20}
JULIA_SCRIPT=${JULIA_SCRIPT:-2d_windowed_cnot_twopass.jl}
THREADS_PER_TASK=${THREADS_PER_TASK:-${SLURM_CPUS_PER_TASK:-8}}
SUBMIT_DIR=${SLURM_SUBMIT_DIR:-$(pwd)}
PROJECT_DIR=${PROJECT_DIR:-${SUBMIT_DIR}}
OUTPUT_DIR=${OUTPUT_DIR:-results/cnot_twopass/initial_scan/T∕2_CNOT_T∕2_2T}
...
P_LIST=(${P_LIST:-0.006 0.008 0.010 0.012 0.014 0.016 0.018 0.020})
...
CNOT_STYLE=${CNOT_STYLE:-twopass}
ACC_ERRORS=${ACC_ERRORS:-100}
SAMPS=${SAMPS:-0}
TVAL_DEFAULT=${TVAL:-L}
...
if [[ -z "${SLURM_ARRAY_TASK_ID:-}" ]]; then
    ...
    echo "Submitting ${TOTAL_TASKS} twopass CNOT full-scan tasks"
...
export OUT_ADJ="_cnot_twopass_full_p${PVAL}_L${LVAL}_rep${REPEAT_INDEX}_T${TVAL_VAL}_Tpre${T_PRE_VAL}_Tpost${T_POST_VAL}${SAMPLE_ADJ}"
export OUTPUT_FILE="${TASK_OUTPUT_DIR}/2d_CNOT_twopass_Ft${OUT_ADJ}.txt"
