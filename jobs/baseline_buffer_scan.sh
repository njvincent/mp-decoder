#!/bin/bash
#SBATCH --job-name=baseline_buffer_scan_threaded
...
#SBATCH --mem=16G
#SBATCH --output=logs/baseline_buffer_scan_threaded_%A_%a.out
#SBATCH --error=logs/baseline_buffer_scan_threaded_%A_%a.err
...
OUTPUT_DIR=${OUTPUT_DIR:-results/baseline/ft/buffer_scan}
...
UPDATE_TIMES=(${UPDATE_TIME_LIST:-1})
CLEANUP_TIMES=(${CLEANUP_TIME_LIST:-2})
BUFFER_DEPTHS=(${BUFFER_DEPTH_LIST:-1 2 3 4 5})
