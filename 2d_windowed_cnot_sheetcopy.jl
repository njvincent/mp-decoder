...
# Sheet-copy second-version CNOT driver.
#
# This file keeps the one-sector memory decoder from
# 2d_windowed_simulation_thread.jl and replaces the primitive shared-field CNOT
# merge with explicit X-error sheet lineages. For CNOT control c -> target t,
# the X-sector rule is X_c -> X_c X_t and X_t -> X_t. Operationally: control
# sheets remain on the control, active control sheets are deep-copied to the
# target, target sheets are not copied back, and no Z sector is implemented.
#
# The crucial invariant is that decoder sheets never exchange hist/field data
# during local decoding. The only algebraic merge is at final readout, where
# decoded states from sheets assigned to the same output block are XORed.
...
function main()
    ...
    * "stats":  get anyon density in the long time steady state
    ...
    CNOT_STYLE = get(ENV, "CNOT_STYLE", "sheetcopy")
    ...
    data_keys = ["Ft" "CNOT_Ft" "CNOT_fail_rate" "trials" "logical_failures" "control_logical_failures" "target_logical_failures" "both_logical_failures" "cleanup_failures" "sheetcopy_final_sheet_count_mean" "sheetcopy_final_active_sheet_count_mean" "sheetcopy_max_sheet_count" "sheetcopy_max_active_sheet_count" "sheetcopy_first_trial_sheet_count_trace" "sheetcopy_first_trial_active_sheet_count_trace" "sheetcopy_sheet_count_trace_note" "demo_files" "demo_visualizer_commands"] ∪ ["hist" "field_hist" "state_hist"] ∪ ["trels" "trel_stats"] ∪ ["erode_times" "erode_stats"] ∪ ["Ms" "binds" "chis" "anyon_densities"] ∪ ["tpreps" "quenched_anyon_densities" "dectest_times" "tprep_errors"]
    ...
    elseif mode == "CNOT_Ft"
        ...
        println("running sheet-copy CNOT fixed-time fidelity test...")
        cnot_data = estimate_sheetcopy_cnot_Ft(L,Z,p,p*qrat,r,synch,pretty,T_PRE,T_POST,CLEANUP_TIME,accu_errors_vec[1],cnot_samps_env,trial_parallel,verbose)
...
function parameter_repository(mode,L,Z,p,qrat,r,synch,vary_L,vary_Z,logZ)
    ...
    if mode == "CNOT_Ft"
        ...
        accu_errors_vec = [1000 for i in 1:nps]
        println("number of sheet-copy CNOT failures to accumulate: ", accu_errors_vec)
    end
